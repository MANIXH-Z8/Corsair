...
Workflow: `discovery → approval → upload/profile → data_ready → queued/running → completed|failed`. Every candidate uses a scikit-learn pipeline, ensuring imputation, encoding, and scaling are fit within cross-validation folds.
